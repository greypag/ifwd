...
<div class="modal fade bs-example-modal-lg " id="p-loginpopup"
        ...
        <div class="modal-dialog modal-lg">
           ...
           <!-- 登入 -->      
                ...
                <form name="p-loginform" id="p-loginform-pop">
               ...
               <img style="width: 100px; height: 100px; position: absolute; top: 40%; left: 40%" src="<%=request.getContextPath()%>/resources/images/ajax-loader.gif">
                                ...
                                <div id="p-login-err-msg" class="alert alert-danger empHide"
                                ...
                                <div class="form-container" >
                                    ...
                                    <div class="form-group">
                                        <input type="text" name="userName" class="form-control check-emp"
                                            placeholder="" id="pUserName" onkeypress="return validationUsername(event);">
                                    </div>
                                    <span id="errPUserName" class="empHide" style="color: red;font-size: 16px;"></span>
                                    ...
                                    <div class="form-group">
                                        <input type="password" name="password" class="form-control check-emp-forgotusername" autocomplete="off" id="pPassword">
                                    </div>
                                    <span id="errpPass" class="empHide" style="color: red;font-size: 16px;"></span> <br>
                                    <div class="row">
...
function submitGo() {
	...
	$('#p-loginform-pop #p-login-err-msg').html("");
	$('#p-loginform-pop #p-login-err-msg').hide();
    setTimeout(function(){
		if (validForm()) {
			$.ajax({
				type : "POST",
				url : "<%=request.getContextPath()%>/userLogin",
				data : $("#p-loginform-pop").serialize(),
				async : false,
				success : function(data) {				
					if (data == 'success') {
						$('#p-loginpopup').modal('hide');
		                ...
		                $('#ajax-loading').hide();
		                $('#p-login-err-msg').show();
		                $('#p-login-err-msg').html(getBundle(getBundleLanguage, "member.login.fail.first"));
		                ...
		                $('#ajax-loading').hide();
		                $('#p-login-err-msg').show();
		                $('#p-login-err-msg').html(getBundle(getBundleLanguage, "member.forgotPassword.success.message"));
					} else if (data == 'Internet Connection Error') {
						try{$('.login-ajax-loading').hide();}catch(error){}
		                $('#ajax-loading').hide();
		                $('#p-login-err-msg').show();
		                $('#p-login-err-msg').html(getBundle(getBundleLanguage, "Connection.lost.message"));
		                ...
		                $('#ajax-loading').hide();
		                $('#p-login-err-msg').show();
		                $('#p-login-err-msg').html(getBundle(getBundleLanguage, "member.login.fail.second"));
					} else if (data == 'Your username has been locked out, please reset your password by \'Forget Password\'.') {
						try{$('.login-ajax-loading').hide();}catch(error){}
		                $('#ajax-loading').hide();
		                $('#p-login-err-msg').show();
		                $('#p-login-err-msg').html(getBundle(getBundleLanguage, "member.login.fail.third"));					
					} else if (data == 'fail') {
...
function validForm()
{
	...
	if (password.trim() == "")
	{    	
		$("#pPassword").addClass("invalid-field");
		$("#errpPass").html(getBundle(getBundleLanguage, "user.password.notNull.message"));
		$("#errpPass").attr("style","color: red;");
		flag = false;
	} 
	if (userName.trim() == "") {
		$("#pUserName").addClass("invalid-field");
		$("#errPUserName").html(getBundle(getBundleLanguage, "user.username.empty.message"));
		$("#errPUserName").attr("style","color: red;");
		flag = false;
	}
	return flag;
}
                ...
                <div id="forgotusername-err-msg" class="alert alert-danger"
                    ...
                    <div id="success-message" class="alert alert-success empHide" role="alert"
                        ...
                        <P id="error1"></P>
                                            ...
                                            <div id="p-login-err-msg" class="alert alert-danger"
                                                role="alert" style="display: none;"></div>
