...
function getSaviePlanDetails() {
	...
	if(premium ==null || premium <30000 || premium > 400000){
		...
		$.get(contextPath+'/ajax/savings-evergreen-insurance/planDetails/get',
		{ 
			planCode : planCode,
			issueAge: issueAge,
			premium: premium,
...
function createPdf() {
	...
	$.get(contextPath+'/ajax/savings-evergreen-insurance/createPdf',
	{ 
	...
	function(data) {
		if(data.pdfName){
			window.open(contextPath+'/tc/savings-evergreen-insurance/pdf-show?pdfName='+data.pdfName+'&requestNo='+data.Msgs.requestNo);
		}
		else{
		}
	})
...
function acceptPdf(pdfName,requestNo) {
	$.get(contextPath+'/ajax/savings-evergreen-insurance/uploadPdf',
	{ 
		pdfName : pdfName,
		requestNo : requestNo
	},
...
function getOccupation(value,language) {
	$.get(contextPath+'/ajax/savings-evergreen-insurance/application/getOccupation',
	{ 
		value : value,
		language : language
	},
