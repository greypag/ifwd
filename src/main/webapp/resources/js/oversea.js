...
$(function() {
	...
	if (!isMobile) {
		...
		if (floatingbox[0]) {
			var floatingboxY = floatingbox.offset().top, wrapboxY = $(
					"#quote-wrap").offset().top;
			var topMargin = 40;

			$(window).on(
					'scroll',
					function(event) {
						if ($("#quote-wrap").height() > $(".floatingbox")
								.height()) {
							var scrollTop = $(window).scrollTop();
							var wrapboxRY = wrapboxY
									+ $("#quote-wrap").height();
							var floatingboxRY = scrollTop
									+ $(".floatingbox").height() + topMargin;

							if (wrapboxRY > floatingboxRY) {
								floatingbox.stop(false).animate(
										{
											top : scrollTop < floatingboxY ? 0
													: scrollTop - floatingboxY
															+ topMargin
										}, 100);
							} else {
								if (floatingboxRY > wrapboxRY) {
									floatingbox.css("top", $("#quote-wrap")
											.height()
											- $(".floatingbox").height())
								}
							}
						...
						if (seccode.trim() == "") {
							$("#errcode")
									.html(
											getBundle(getBundleLanguage,
													"payment.creditCard.securityCode.notNull.message"));// "Please
																										// enter
																										// your
																										// Name
																										// in
							...
							if (seccode.length < 3) {
								$('#errcode')
										.html(
												getBundle(getBundleLanguage,
														"payment.creditCard.securityCode.notValid.message"));
								$("#seccode").addClass("invalid-field");
								return false;
							}
						}
	...
	var now = new Date(nowTemp.getFullYear(), nowTemp.getMonth(), nowTemp
			.getDate(), 0, 0, 0, 0);
	var tillDate_from = new Date((new Date()).getTime() + 89 * 24 * 60 * 60
			* 1000);
	var tillDate_from_travel = new Date((new Date()).getTime() + 179 * 24 * 60
			* 60 * 1000);
	var duration = $('#frmTravelGetQuote').length > 0
			|| $('#frmTravelPlan').length > 0 ? 180 * 24 * 60 * 60 * 1000 : 30
			* 24 * 60 * 60 * 1000;
	var oneDay = 24 * 60 * 60 * 1000;

	var checkout;
	/* desktoip datepicker */
	$("#txtStartDateDesk").blur(
			function() {
				var dateDiff = dateDiffInDaysFromNow(checkin.datepicker(
						"getDate").valueOf());
				if (dateDiff < 0) {
					this.focus();
					return false;
	...
	$("#txtEndDateDesk").blur(
			function() {
				var dateDiff = dateDiffInDaysFromNow(checkout.datepicker(
						"getDate").valueOf());
				if (dateDiff < 0) {
					this.focus();
					return false;
	...
	$("#txtStartDateMob").blur(
			function() {
				var dateDiff = dateDiffInDaysFromNow(checkin2.datepicker(
						"getDate").valueOf());
				if (dateDiff < 0) {
					this.focus();
					return false;
	...
	$("#txtEndDateMob").blur(
			function() {
				var dateDiff = dateDiffInDaysFromNow(checkout2.datepicker(
						"getDate").valueOf());
				if (dateDiff < 0) {
					this.focus();
					return false;
	...
	$("#txtStartDateBtm").blur(
			function() {
				var dateDiff = dateDiffInDaysFromNow(checkin3.datepicker(
						"getDate").valueOf());
				if (dateDiff < 0) {
					this.focus();
					return false;
	...
	$("#txtEndDateBtm").blur(
			function() {
				var dateDiff = dateDiffInDaysFromNow(checkout3.datepicker(
						"getDate").valueOf());
				if (dateDiff < 0) {
					this.focus();
					return false;
	...
	checkoutTravelDetail = $('#trval-dp2').datepicker(
			{
				beforeShowDay : function(date) {
					if (!checkinTravelDetail.datepicker("getDate").valueOf()) {
						...
						return date.valueOf() >= checkinTravelDetail
								.datepicker("getDate").valueOf()
								&& date.valueOf() < checkinTravelDetail
										.datepicker("getDate").valueOf()
										+ duration;
					}
				},
			...
			function(ev) {
				...
				var startDate = new Date($('#trval-dp1').datepicker("getDate")
						.valueOf());
				var endDate = new Date($('#trval-dp2').datepicker("getDate")
						.valueOf());
...
//			});
	...
	checkoutFightDetail = $('#dp2-detail-page').datepicker(
			{
				beforeShowDay : function(date) {
					if (!checkinFightDetail.datepicker("getDate").valueOf()) {
						...
						return date.valueOf() >= checkinFightDetail.datepicker(
								"getDate").valueOf()
								&& date.valueOf() < checkinFightDetail
										.datepicker("getDate").valueOf()
										+ duration;
					}
				},
			...
			function(ev) {
				...
				var startDate = new Date($('#dp1-detail-page').datepicker(
						"getDate").valueOf());
				var endDate = new Date($('#dp2-detail-page').datepicker(
						"getDate").valueOf());
	...
	$('#input_dob,#input_dob2,#input_dob3').datepicker({
		startView : "decade",
		autoclose : true,
		format : "dd-mm-yyyy",
	...
	/* language: getBundleLanguage */
			...
			function(ev) {
				...
				if (ev.date != undefined) {
					if (ev.date.valueOf() < dob_end_date.valueOf()
							&& ev.date.valueOf() > dob_70_date.valueOf()) {
						selected = 2;
					} else {
						selected = 3;
					}
	...
	$('#input_dob,#input_dob2,#input_dob3').datepicker('setDate', dob_end_date);
	...
	$('#input_annual_dob').datepicker({
		...
		endDate : dob_end_date
			...
			'changeDate',
			function(ev) {
				$('#input_insure_dob1').datepicker('setDate',
						$("#applicantDob").val());
				$('#input_adult_dob1').datepicker('setDate',
						$("#applicantDob").val());
				$("#dobInvalid").html("");
				$("#errtxtInsuDob1").html("");
				$("#errtxtAdDob1").html("");
	...
	/* language: getBundleLanguage */
		...
		// var selected = 2;
		// if(ev.date != undefined) {
		// if(ev.date.valueOf() < dob_end_date.valueOf() && ev.date.valueOf() >
		// dob_70_date.valueOf()){
		// selected = 2;
		// }else{
		// selected = 3;
	...
	var checkin = $('#dp1').datepicker(
			...
			function(ev) {
				...
				if (ev.date.valueOf() > checkout.datepicker("getDate")
						.valueOf()
						|| !checkout.datepicker("getDate").valueOf()) {
					checkout.datepicker("update", newDate);
					checkout2.datepicker("update", newDate);
					checkout3.datepicker("update", newDate);
				} else if (ev.date.valueOf() + duration <= checkout.datepicker(
						"getDate").valueOf()) {
					var lastDate = new Date(ev.date.valueOf() + duration
							- oneDay);
					...
					var lastDate = new Date(checkout.datepicker("getDate")
							.valueOf());
					lastDate.setDate(lastDate.getDate());
				...
				// checkout.datepicker("setEndDate", new
				// Date(checkin.datepicker("getDate").valueOf() + duration));

				// }

				$('#dp2')[0].focus();
				var startDate = new Date($('#dp1').datepicker("getDate")
						.valueOf());
				var endDate = new Date($('#dp2').datepicker("getDate")
						.valueOf());
				/*
				 * document.getElementById("divPersonsDesk").style.visibility =
				 * "visible"; document.getElementById("lblDaysDesk").innerHTML =
				 * isNaN(dateDiffInDays(startDate, endDate)) ? 0 :
				 * dateDiffInDays(startDate, endDate);
				 */
	...
	checkout = $('#dp2').datepicker(
				...
				beforeShowDay : function(date) {
					if (!checkin.datepicker("getDate").valueOf()) {
						...
						return date.valueOf() >= checkin.datepicker("getDate")
								.valueOf()
								&& date.valueOf() < checkin.datepicker(
										"getDate").valueOf()
										+ duration;
					}
				},
			...
			function(ev) {
				...
				if (ev.date.valueOf() < checkin.datepicker("getDate").valueOf()
						|| !checkin.datepicker("getDate").valueOf()) {
	...
	var checkin2 = $('#dp3')
			...
			.on(
					...
					function(ev) {
						// if (ev.date.valueOf() >
						// checkout2.datepicker("getDate").valueOf() ||
						// !checkout2.datepicker("getDate").valueOf()) {
						var newDate = new Date(ev.date);
						newDate.setDate(newDate.getDate());
						...
						if (ev.date.valueOf() > checkout2.datepicker("getDate")
								.valueOf()
								|| !checkout2.datepicker("getDate").valueOf()) {
							checkout.datepicker("update", newDate);
							checkout2.datepicker("update", newDate);
							checkout3.datepicker("update", newDate);
						} else if (ev.date.valueOf() + duration <= checkout2
								.datepicker("getDate").valueOf()) {
							var lastDate = new Date(ev.date.valueOf()
									+ duration - oneDay);
							lastDate.setDate(lastDate.getDate());
							...
							var lastDate = new Date(checkout2.datepicker(
									"getDate").valueOf());
							lastDate.setDate(lastDate.getDate());
						...
						var startDate = new Date($('#dp3')
								.datepicker("getDate").valueOf());
						var endDate = new Date($('#dp4').datepicker("getDate")
								.valueOf());
						document.getElementById("divPersonsMob").style.visibility = "visible";
						document.getElementById("lblDaysMob").innerHTML = isNaN(dateDiffInDays(
								startDate, endDate)) ? 0 : dateDiffInDays(
								startDate, endDate);
	...
	var checkout2 = $('#dp4')
			.datepicker(
					{
						beforeShowDay : function(date) {
							...
							// } else {
							// return date.valueOf() >=
							// checkin2.datepicker("getDate").valueOf();
							// }
							if (!checkin2.datepicker("getDate").valueOf()) {
								...
								return date.valueOf() >= checkin2.datepicker(
										"getDate").valueOf()
										&& date.valueOf() < checkin2
												.datepicker("getDate")
												.valueOf()
												+ duration;
							}
			...
			.on(
					...
					function(ev) {
						...
						if (ev.date.valueOf() < checkin2.datepicker("getDate")
								.valueOf()
								|| !checkin2.datepicker("getDate").valueOf()) {
							checkin.datepicker("update", endDate);
							checkin2.datepicker("update", endDate);
	...
	var checkin3 = $('#dp5')
			...
			.on(
					...
					function(ev) {
						// if (ev.date.valueOf() >
						// checkout3.datepicker("getDate").valueOf() ||
						// !checkout3.datepicker("getDate").valueOf()) {
						var newDate = new Date(ev.date);
						newDate.setDate(newDate.getDate());

						if (ev.date.valueOf() > checkout3.datepicker("getDate")
								.valueOf()
								|| !checkout3.datepicker("getDate").valueOf()) {
							checkout.datepicker("update", newDate);
							checkout2.datepicker("update", newDate);
							checkout3.datepicker("update", newDate);
						} else if (ev.date.valueOf() + duration <= checkout3
								.datepicker("getDate").valueOf()) {
							var lastDate = new Date(ev.date.valueOf()
									+ duration - oneDay);
							lastDate.setDate(lastDate.getDate());
							...
							var lastDate = new Date(checkout3.datepicker(
									"getDate").valueOf());
							lastDate.setDate(lastDate.getDate());
						...
						// }
						$('#dp6')[0].focus();
						var startDate = new Date($('#dp5')
								.datepicker("getDate").valueOf());
						var endDate = new Date($('#dp6').datepicker("getDate")
								.valueOf());
						document.getElementById("divPersonsBtm").style.visibility = "visible";
						document.getElementById("lblDaysBtm").innerHTML = isNaN(dateDiffInDays(
								startDate, endDate)) ? 0 : dateDiffInDays(
								startDate, endDate);
	...
	var checkout3 = $('#dp6')
			.datepicker(
					{
						beforeShowDay : function(date) {
							...
							// } else {
							// return date.valueOf() >=
							// checkin3.datepicker("getDate").valueOf();
							// }
							if (!checkin3.datepicker("getDate").valueOf()) {
								...
								return date.valueOf() >= checkin3.datepicker(
										"getDate").valueOf()
										&& date.valueOf() < checkin3
												.datepicker("getDate")
												.valueOf()
												+ duration;
							}
			...
			.on(
					...
					function(ev) {
						...
						if (ev.date.valueOf() < checkin3.datepicker("getDate")
								.valueOf()
								|| !checkin3.datepicker("getDate").valueOf()) {
							checkin.datepicker("update", endDate);
							checkin2.datepicker("update", endDate);
	...
	var tcheckin = $('#tdp1').datepicker(
			...
			function(ev) {
				...
				if (ev.date.valueOf() > tcheckout.datepicker("getDate")
						.valueOf()
						|| !tcheckout.datepicker("getDate").valueOf()) {
					tcheckout.datepicker("update", newDate);
					tcheckout2.datepicker("update", newDate);
					tcheckout3.datepicker("update", newDate);
				} else if (ev.date.valueOf() + duration <= tcheckout
						.datepicker("getDate").valueOf()) {
					var lastDate = new Date(ev.date.valueOf() + duration
							- oneDay);
					lastDate.setDate(lastDate.getDate());
					...
					var lastDate = new Date(tcheckout.datepicker("getDate")
							.valueOf());
					lastDate.setDate(lastDate.getDate());
				...
				$('#tdp2')[0].focus();
				var startDate = new Date($('#tdp1').datepicker("getDate")
						.valueOf());
				var endDate = new Date($('#tdp2').datepicker("getDate")
						.valueOf());
				var path = window.location.href;
				var end = path.indexOf('flight-insurance');
				var fullPath = "";
	...
	tcheckout = $('#tdp2').datepicker(
			{
				beforeShowDay : function(date) {
					if (!tcheckin.datepicker("getDate").valueOf()) {
						return date.valueOf() >= new Date().valueOf()
								&& date.valueOf() < tillDate_from_travel;
					} else {
						return date.valueOf() >= tcheckin.datepicker("getDate")
								.valueOf()
								&& date.valueOf() < tcheckin.datepicker(
										"getDate").valueOf()
										+ 180 * 24 * 60 * 60 * 1000;
					}
				},
	...
	var tcheckin2 = $('#tdp3')
			...
			.on(
					...
					function(ev) {
						...
						$('#tdp5').datepicker('update', newDate);
						if (ev.date.valueOf() > tcheckout2
								.datepicker("getDate").valueOf()
								|| !tcheckout2.datepicker("getDate").valueOf()) {
							tcheckout.datepicker("update", newDate);
							tcheckout2.datepicker("update", newDate);
							tcheckout3.datepicker("update", newDate);
						} else if (ev.date.valueOf() + duration <= tcheckout2
								.datepicker("getDate").valueOf()) {
							var lastDate = new Date(ev.date.valueOf()
									+ duration - oneDay);
							lastDate.setDate(lastDate.getDate());
							...
							var lastDate = new Date(tcheckout2.datepicker(
									"getDate").valueOf());
							lastDate.setDate(lastDate.getDate());
							tcheckout.datepicker("update", lastDate);
							tcheckout2.datepicker("update", lastDate);
						...
						$('#tdp4')[0].focus();
						var startDate = new Date($('#tdp3').datepicker(
								"getDate").valueOf());
						var endDate = new Date($('#tdp4').datepicker("getDate")
								.valueOf());
						document.getElementById("divPersonsMob").style.visibility = "visible";
						document.getElementById("lblDaysMob").innerHTML = isNaN(dateDiffInDays(
								startDate, endDate)) ? 0 : dateDiffInDays(
			...
			.datepicker(
					{
						beforeShowDay : function(date) {
							if (!tcheckin2.datepicker("getDate").valueOf()) {
								...
								return date.valueOf() >= tcheckin2.datepicker(
										"getDate").valueOf()
										&& date.valueOf() < tcheckin2
												.datepicker("getDate")
												.valueOf()
												+ 180 * 24 * 60 * 60 * 1000;
								;
	...
	var tcheckin3 = $('#tdp5')
			...
			.on(
					...
					function(ev) {
						...
						if (ev.date.valueOf() > tcheckout3
								.datepicker("getDate").valueOf()
								|| !tcheckout3.datepicker("getDate").valueOf()) {
							tcheckout.datepicker("update", newDate);
							tcheckout2.datepicker("update", newDate);
							tcheckout3.datepicker("update", newDate);
						} else if (ev.date.valueOf() + duration <= tcheckout3
								.datepicker("getDate").valueOf()) {
							var lastDate = new Date(ev.date.valueOf()
									+ duration - oneDay);
							lastDate.setDate(lastDate.getDate());
							...
							var lastDate = new Date(tcheckout3.datepicker(
									"getDate").valueOf());
							lastDate.setDate(lastDate.getDate());
						...
						$('#tdp3').datepicker('update', newDate);
						$('#tdp6')[0].focus();
						var startDate = new Date($('#tdp5').datepicker(
								"getDate").valueOf());
						var endDate = new Date($('#tdp6').datepicker("getDate")
								.valueOf());
						document.getElementById("divPersonsBtm").style.visibility = "visible";
						document.getElementById("lblDaysBtm").innerHTML = isNaN(dateDiffInDays(
								startDate, endDate)) ? 0 : dateDiffInDays(
			...
			.datepicker(
					{
						beforeShowDay : function(date) {
							if (!tcheckin3.datepicker("getDate").valueOf()) {
								...
								return date.valueOf() >= tcheckin3.datepicker(
										"getDate").valueOf()
										&& date.valueOf() < tcheckin3
												.datepicker("getDate")
												.valueOf()
												+ 180 * 24 * 60 * 60 * 1000;
								;
...
$(
		'#myFWDropdown .dropdown-menu,#myFWDropdownMob .dropdown-menu,#myFWDropdownBtm .dropdown-menu')
		.on({
			"click" : function(e) {
				e.stopPropagation();
			}
...
$(
		'#travelTypeDropdown .dropdown-menu,#travelTypeDropdownMob .dropdown-menu,#travelTypeDropdownBtm .dropdown-menu')
		.on({
			"click" : function(e) {
				e.stopPropagation();
			}
...
function fPlanValid() {
	...
	var insuredDob = document.getElementById("insuredDob").value;
	var inputOverseasInsuredDob = document
			.getElementById("inputOverseasInsuredDob").value;
	// first error element
	...
	if (appHkid.trim() == "") {
		document.getElementById("errAppHkid").innerHTML = getBundle(
				getBundleLanguage, "applicant.hkId.notNull.message");
		...
		if (tr == false) {
			...
			document.getElementById("errAppHkid").innerHTML = getBundle(
					getBundleLanguage, "applicant.hkId.notValid.message");
			$("#inputTxtAppHkid").addClass("invalid-field");
			if (firstErrorElementId == "") {
				firstErrorElementId = "inputTxtAppHkid";
			}
	...
	if (applicantDob.trim() == "") {
		document.getElementById("dobInvalid").innerHTML = getBundle(
				getBundleLanguage, "applicant.dob.notNull.message");
		...
		var applicantDob1 = new Date(applicantDobs[2], applicantDobs[1] - 1,
				applicantDobs[0], 0, 0, 0, 0);
		...
		if (age == 1) {
			if (difference > 18) {
				$('#dobInvalid').html(
						getBundle(getBundleLanguage,
								"applicant.dob.notValid.message"));
				$("#input_dob").addClass("invalid-field");
				if (firstErrorElementId == "") {
					firstErrorElementId = "input_dob";
				}
	...
	if (mobileNo.trim() == "") {
		document.getElementById("mobileNoInvalid").innerHTML = getBundle(
				getBundleLanguage, "applicant.mobileNo.notNull.message");
		...
		if (mobile_pattern.test(mobileNo) == false) {
			document.getElementById("mobileNoInvalid").innerHTML = getBundle(
					getBundleLanguage, "applicant.mobileNo.notValid.message");
			$("#inputMobileNo").addClass("invalid-field");
			if (firstErrorElementId == "") {
				firstErrorElementId = "inputMobileNo";
			}
	...
	if (emailId.trim() == "") {
		document.getElementById("emailid").innerHTML = getBundle(
				getBundleLanguage, "applicant.email.notNull.message");
		...
		if (emailreg.test(emailId) == false) {
			document.getElementById("emailid").innerHTML = getBundle(
					getBundleLanguage, "applicant.email.notValid.message");
			$("#inputEmailId").addClass("invalid-field");
			if (firstErrorElementId == "") {
				firstErrorElementId = "inputEmailId";
			}
	...
	for (var i = 1; i <= parseInt(rowCountPersonal); i++) {
		if ($("#txtInsuFullName" + i).val().trim() == insureNamePlaceholder
				.trim()) {
		...
		if (fullname.trim() == "") {
			document.getElementById("errtxtPersonalFullName" + i).innerHTML = getBundle(
					getBundleLanguage, "insured.name.notNull.message"); // "Please
																		// enter
																		// Insured
																		// Person's
																		// Name
		...
		if (age.trim() == "" || age.trim() == 0) {
			document.getElementById("errselectAgeRange" + i).innerHTML = getBundle(
					getBundleLanguage, "insured.age.notValid.message");
		...
		if (hkid.trim() == "") {
			document.getElementById("errtxtInsuHkid" + i).innerHTML = getBundle(
					getBundleLanguage, "insured.hkId.notNull.message"); // "Please
																		// enter
																		// Insured
																		// Person's
																		// HKID
			...
			if (tr == false) {
				document.getElementById("errtxtInsuHkid" + i).innerHTML = getBundle(
						getBundleLanguage, "insured.hkId.notValid.message"); // getBundle(getBundleLanguage,
																				// "insured.hkId.notValid.message");;
				$("#txtInsuHkid" + i).addClass("invalid-field");
				if (firstErrorElementId == "") {
					firstErrorElementId = "txtInsuHkid" + i;
				}
		...
		if (hkid.trim() != "") {
			for (var j = 1; j <= i - 1; j++) {
				...
				if (hkid.toUpperCase() == hkid1.toUpperCase()) {
					$('#errtxtInsuHkid' + j).html(
							getBundle(getBundleLanguage,
									"insured.hkId.duplicate.meesage"));
		...
		var selectedValue = document.getElementById("personalselectBenificiary"
				+ i).value;
		var HkidPass = document.getElementById("personalBenefitiaryHKId" + i).value;
		var selectPersonalBenefitiaryHkidPass = document
				.getElementById("selectPersonalBenefitiaryHkidPass" + i).value;

		if ($("#personalBenefitiaryId" + i).val().trim() == benNamePlaceholder
				.trim()) {
			$("#personalBenefitiaryId" + i).val('');
		}
		if ($("#personalBenefitiaryHKId" + i).val().trim() == benHkidPlaceholder
				.trim()) {
		...
		if (selectedValue != "SE") {
			...
			// if (HkidPass.trim() == "") {
			// document.getElementById("errpersonalBenefitiaryHKId" +
			// i).innerHTML = getBundle(getBundleLanguage,
			// "beneficiary.hkId.notNull.message")
			// flag = false;
			// }
			// else
			// {
			...
			// document.getElementById("errpersonalBenefitiaryHKId" +
			// i).innerHTML = getBundle(getBundleLanguage,
			// "beneficiary.hkId.notValid.message");;
			// flag = false;
			// }
			// }
			//			

			if (document.getElementById("personalBenefitiaryId" + i).value
					.trim() == "") {
				document.getElementById("errpersonalBenefitiaryId" + i).innerHTML = getBundle(
						getBundleLanguage, "beneficiary.name.notNull.message"); // getBundle(getBundleLanguage,
																				// "beneficiary.name.notNull.message");;
			...
			if (selectPersonalBenefitiaryHkidPass.toUpperCase() == 'HKID') {
				if (hkida.trim() == "") {
					$("#errpersonalBenefitiaryHKId" + i).html(
							getBundle(getBundleLanguage,
									"beneficiary.hkId.notNull.message"));
					...
					if (IsHKID(hkida.trim()) == false) {
						$("#errpersonalBenefitiaryHKId" + i).html(
								getBundle(getBundleLanguage,
										"beneficiary.hkId.notValid.message"));
						$("#personalBenefitiaryHKId" + i).addClass(
								"invalid-field");
						if (firstErrorElementId == "") {
							firstErrorElementId = "personalBenefitiaryHKId" + i;
						}
						...
						$('#errpersonalBenefitiaryHKId' + i).html(
								getBundle(getBundleLanguage,
										"beneficiary.hkId.duplicate.message"));
						$("#personalBenefitiaryHKId" + i).addClass(
								"invalid-field");
						if (firstErrorElementId == "") {
							firstErrorElementId = "personalBenefitiaryHKId" + i;
				...
				if (hkida.trim() == "") {
					document.getElementById("errpersonalBenefitiaryHKId" + i).innerHTML = getBundle(
							getBundleLanguage,
							"insured.passport.notNull.message"); // "Please
																	// enter
																	// Insured
																	// Person's
																	// HKID
					...
					if (chkTravelHKPass(hkida.trim()) == false) {
						$("#errpersonalBenefitiaryHKId" + i)
								.html(
										getBundle(getBundleLanguage,
												"beneficiary.passport.notValid.message"));
						$("#personalBenefitiaryHKId" + i).addClass(
								"invalid-field");
						if (firstErrorElementId == "") {
							firstErrorElementId = "personalBenefitiaryHKId" + i;
					...
					if (chkTravelHKPassLen(hkida.trim()) == false) {
						$("#errpersonalBenefitiaryHKId" + i)
								.html(
										getBundle(getBundleLanguage,
												"applicant.passport.notValidLength.message"));
						$("#personalBenefitiaryHKId" + i).addClass(
								"invalid-field");
						if (firstErrorElementId == "") {
							firstErrorElementId = "personalBenefitiaryHKId" + i;
	...
	for (var i = 1; i <= parseInt(rowCountAdult); i++) {
		...
		var fullname = document.getElementById("txtInsuFullName" + i).value;
		if (fullname.trim() == "") {
			document.getElementById("errtxtAdFullName" + i).innerHTML = getBundle(
					getBundleLanguage, "insured.name.notNull.message"); // "Please
																		// enter
																		// Insured
																		// Person's
		...
		if (hkid.trim() == "") {
			document.getElementById("errtxtInsuHkid" + i).innerHTML = getBundle(
					getBundleLanguage, "insured.hkId.notNull.message")
			$("#txtInsuHkid" + i).addClass("invalid-field");
			if (firstErrorElementId == "") {
				firstErrorElementId = "txtInsuHkid" + i;
			}
			...
			if (tr == false) {
				document.getElementById("errtxtInsuHkid" + i).innerHTML = getBundle(
						getBundleLanguage, "insured.hkId.notValid.message");
				;
				$("#txtInsuHkid" + i).addClass("invalid-field");
				if (firstErrorElementId == "") {
					firstErrorElementId = "txtInsuHkid" + i;
		...
		var selectedValue = document.getElementById("adultsselectBenificiary"
				+ i).value;
		var HkidPass = document.getElementById("adultBenefitiaryHKId" + i).value;

		var selectAdBenefitiaryHkidPass = document
				.getElementById("selectAdBenefitiaryHkidPass" + i).value;

		if ($("#adultBenefitiaryId" + i).val().trim() == benNamePlaceholder
				.trim()) {
			$("#adultBenefitiaryId" + i).val('');
		}
		if ($("#adultBenefitiaryHKId" + i).val().trim() == benHkidPlaceholder
				.trim()) {
		...
		if (selectedValue != "SE") {
			...
			if (document.getElementById("adultBenefitiaryId" + i).value == "") {
				document.getElementById("erradultBenefitiaryId" + i).innerHTML = getBundle(
						getBundleLanguage, "beneficiary.name.notNull.message"); // getBundle(getBundleLanguage,
																				// "beneficiary.name.notNull.message");;
			...
			if (selectAdBenefitiaryHkidPass.toUpperCase() == 'HKID') {
				...
				if (hkida.trim() == "") {
					document.getElementById("erradultBenefitiaryHKId" + i).innerHTML = getBundle(
							getBundleLanguage,
							"beneficiary.passport.notNull.message"); // "Please
																		// enter
																		// Insured
																		// Person's
																		// HKID
	...
	for (var i = 1; i <= parseInt(rowCountChild); i++) {
		...
		if ($("#txtChldFullName" + i).val().trim() == insureNamePlaceholder
				.trim()) {
			$("#txtChldFullName" + i).val('');
		}
		if ($("#txtChldInsuHkid" + i).val().trim() == insureHkidPlaceholder
				.trim()) {
		...
		if (fullname.trim() == "") {
			document.getElementById("errtxtChldFullName" + i).innerHTML = getBundle(
					getBundleLanguage, "insured.name.notNull.message");
		...
		if (age.trim() == "" || age.trim() == 0) {
			document.getElementById("errchildRange" + i).innerHTML = getBundle(
					getBundleLanguage, "insured.age.notValid.message"); // getBundle(getBundleLanguage,
																		// "insured.age.notValid.message");;
		...
		if (hkid.trim() == "") {
			document.getElementById("errtxtChldInsuHkid" + i).innerHTML = getBundle(
					getBundleLanguage, "insured.hkId.notNull.message")
		...
		var selectedValue = document.getElementById("childselectBenificiary"
				+ i).value;
		var HkidPass = document.getElementById("txtchildInsuHkid" + i).value;
		var selectedChldBenefitiaryHkidPass = document
				.getElementById("selectChldBenefitiaryHkidPass" + i).value;

		if ($("#childBenefitiaryName" + i).val().trim() == benNamePlaceholder
				.trim()) {
			$("#childBenefitiaryName" + i).val('');
		}
		if ($("#txtchildInsuHkid" + i).val().trim() == benHkidPlaceholder
				.trim()) {
		...
		if (selectedValue != "SE") {
			...
			// getBundle(getBundleLanguage, "beneficiary.hkId.notNull.message")
			// flag = false;
			// }
			// else
			// {
			...
			if (document.getElementById("childBenefitiaryName" + i).value == "") {
				document.getElementById("errchildBenefitiaryName" + i).innerHTML = getBundle(
						getBundleLanguage, "beneficiary.name.notNull.message"); // getBundle(getBundleLanguage,
																				// "beneficiary.name.notNull.message");;
			...
			if (selectedChldBenefitiaryHkidPass.toUpperCase() == 'HKID') {
				...
				if (hkida.trim() == "") {
					document.getElementById("errtxtchildInsuHkid" + i).innerHTML = getBundle(
							getBundleLanguage,
							"beneficiary.passport.notNull.message"); // "Please
																		// enter
																		// Insured
	...
	for (var i = 1; i <= parseInt(rowCountOther); i++) {
		if ($("#txtOtherFullName" + i).val().trim() == insureNamePlaceholder
				.trim()) {
			$("#txtOtherFullName" + i).val('');
		}
		if ($("#txtOtherInsuHkid" + i).val().trim() == insureHkidPlaceholder
				.trim()) {
		...
		if (fullname.trim() == "") {
			document.getElementById("errtxtOtherFullName" + i).innerHTML = getBundle(
					getBundleLanguage, "insured.name.notNull.message");
		...
		if (age.trim() == "" || age.trim() == 0) {
			document.getElementById("errselectOtherAgeRange" + i).innerHTML = getBundle(
					getBundleLanguage, "insured.age.notValid.message");
		...
		if (hkid.trim() == "") {
			document.getElementById("errtxtOtherInsuHkid" + i).innerHTML = getBundle(
					getBundleLanguage, "insured.hkId.notNull.message")
		...
		if (hkid.trim() != "") {
			for (var j = 1; j <= i - 1; j++) {
				var hkid1 = document.getElementById("txtOtherInsuHkid" + j).value;
				if (hkid.toUpperCase() == hkid1.toUpperCase()) {
					document.getElementById("errtxtOtherInsuHkid" + j).innerHTML = getBundle(
							getBundleLanguage, "insured.hkId.duplicate.meesage");
					$("#txtOtherInsuHkid" + j).addClass("invalid-field");
					if (firstErrorElementId == "") {
						firstErrorElementId = "txtOtherInsuHkid" + j;
					}
		...
		if ($("#otherBenefitiaryName" + i).val().trim() == benNamePlaceholder
				.trim()) {
			$("#otherBenefitiaryName" + i).val('');
		}
		if ($("#txtOtherBenInsuHkid" + i).val().trim() == benHkidPlaceholder
				.trim()) {
			$("#txtOtherBenInsuHkid" + i).val('');
		}

		var selectedValue = document.getElementById("otherSelectBenificiary"
				+ i).value;
		if (selectedValue != "SE") {
			// if (hkid.trim() == "") {
			...
			if (document.getElementById("otherBenefitiaryName" + i).value == "") {
				document.getElementById("errotherBenefitiaryName" + i).innerHTML = getBundle(
						getBundleLanguage, "beneficiary.name.notNull.message");
			...
			var selectOtherBenefitiaryHkidPass = document
					.getElementById("selectOtherBenefitiaryHkidPass" + i).value;
			...
			if (selectOtherBenefitiaryHkidPass.toUpperCase() == 'HKID') {
				...
				if (hkidc.trim() == "") {
					document.getElementById("errtxtOtherBenInsuHkid" + i).innerHTML = getBundle(
							getBundleLanguage,
							"beneficiary.passport.notNull.message"); // "Please
																		// enter
																		// Insured
...
function validateHkid(inputId, selectId, errorId, insureBoolean, inputType) {
	...
	if ($('#' + selectId).length > 0
			&& ($('#' + selectId).val().toLowerCase() == 'passport' || $(
					'#' + selectId).val().toLowerCase() == 'apppassport')) {
		if (appHkid.trim() == "") {
			$("#" + inputId).addClass("invalid-field");
			if (inputType == "applicant") {
				$('#' + errorId).html(
						getBundle(getBundleLanguage,
								"applicant.passport.notNull.message"));
			} else if (inputType == "insured") {
				$("#" + errorId).html(
						getBundle(getBundleLanguage,
								"insured.passport.lengthViolation.message"));
			} else if (inputType == "beneficiary") {
				$("#" + errorId).html(
						getBundle(getBundleLanguage,
								"beneficiary.passport.notNull.message"));
			}
			return false;
		} else {
			...
			if (tr1 == false) {
				...
				if (inputType == "applicant") {
					$('#' + errorId).html(
							getBundle(getBundleLanguage,
									"applicant.passport.notEnglish.message"));
				} else if (inputType == "insured") {
					$("#" + errorId).html(
							getBundle(getBundleLanguage,
									"insured.passport.notEnglish.message"));
				} else if (inputType == "beneficiary") {
					$("#" + errorId).html(
							getBundle(getBundleLanguage,
									"beneficiary.passport.notValid.message"));
		...
		if (appHkid.trim() == "") {
			...
			if (inputType == "applicant") {
				$('#' + errorId).html(
						getBundle(getBundleLanguage,
								"applicant.hkId.notNull.message"));
			} else if (inputType == "insured") {
				$("#" + errorId).html(
						getBundle(getBundleLanguage,
								"insured.hkId.notNull.message"));
			} else if (inputType == "beneficiary") {
				$("#" + errorId).html(
						getBundle(getBundleLanguage,
								"beneficiary.hkId.notNull.message"));
...
$(function() {
	if ($('#inputFullName').length > 0) { // run only for the
		...
		//			
		// if (fullname.trim() == "") {
		// $("#fullnameinvalid").html( getBundle(getBundleLanguage,
		// "applicant.name.notNull.message"));//"Please enter your Name in
		// English.";
		// return false;
		// }
		// if (allLetter(fullname) == false) {
		...
		//				
		// if($('#selectHkidPass').length > 0 &&
		// ($('#selectHkidPass').val().toLowerCase() == 'passport' ||
		// $('#selectHkidPass').val().toLowerCase() == 'apppassport')){
		//
		// if (appHkid.trim() == "") {
		// $('#errAppHkid').html(getBundle(getBundleLanguage,
		// "applicant.missingHkidOrPassport.message"));
		// return false;
		// }else{
		...
		// if (appHkid.trim() == "") {
		// $('#errAppHkid').html(getBundle(getBundleLanguage,
		// "applicant.hkId.notNull.message"));//"Please enter your Name in
		// English.";
		// return false;
		// }
		// var tr=IsHKID(appHkid.trim());
		...
		//			
		// if (emailId.trim() == "") {
		// $("#emailid").html(getBundle(getBundleLanguage,
		// "applicant.email.notNull.message"));
		// return false;
		// } else {
		// if (emailreg.test(emailId) == false) {
		...
		//			
		// if (mobileNo.trim() == "") {
		// $("#mobileNo").html(getBundle(getBundleLanguage,
		// "applicant.mobileNo.notNull.message"));
		// return false;
		// }else {
		// if (mobile_pattern.test(mobileNo) == false) {
		...
		// if (fullname.trim() == "") {
		// $("#errtxtAdFullName"+errNo).html( getBundle(getBundleLanguage,
		// "insured.name.notNull.message"));//"Please enter your Name in
		// English.";
		// return false;
		// }
		...
		//				
		//				
		// if($('#selectedAdHkidPass'+errNo).length > 0 &&
		// $('#selectedAdHkidPass'+errNo).val().toLowerCase() == 'passport'){
		//
		// if (appHkid.trim() == "") {
		// $("#errtxtInsuHkid"+errNo).html(getBundle(getBundleLanguage,
		// "insured.passport.lengthViolation.message"));
		// return false;
		// }else{
		// var tr = chkTravelHKPass(appHkid.trim());
		...
		// if (appHkid.trim() == "") {
		// $("#errtxtInsuHkid"+errNo).html(getBundle(getBundleLanguage,
		// "insured.hkId.notNull.message"));//"Please enter your Name in
		// English.";
		// return false;
		// }
		// var tr=IsHKID(appHkid.trim());
		...
		// var fullname = $(this).val();
		// if (fullname.trim() == "") {
		// $("#errtxtChldFullName"+errNo).html( getBundle(getBundleLanguage,
		// "insured.name.notNull.message"));//"Please enter your Name in
		// English.";
		// return false;
		// }
		...
		//				
		//				
		// if($('#selectChldHkidPass'+errNo).length > 0 &&
		// $('#selectChldHkidPass'+errNo).val().toLowerCase() == 'passport'){
		//
		// if (appHkid.trim() == "") {
		// $("#errtxtChldInsuHkid"+errNo).html(getBundle(getBundleLanguage,
		// "insured.passport.lengthViolation.message"));
		// return false;
		// }else{
		// var tr = chkTravelHKPass(appHkid.trim());
		...
		// var fullname = $(this).val();
		// if (fullname.trim() == "") {
		// $("#errtxtOtherFullName"+errNo).html( getBundle(getBundleLanguage,
		// "insured.name.notNull.message"));//"Please enter your Name in
		// English.";
		// return false;
		// }
		...
		// var appHkid = $(this).val();
		//				
		// if($('#selectOtHkidPass'+errNo).length > 0 &&
		// $('#selectOtHkidPass'+errNo).val().toLowerCase() == 'passport'){
		//
		// if (appHkid.trim() == "") {
		// $("#errtxtOtherInsuHkid"+errNo).html(getBundle(getBundleLanguage,
		// "insured.passport.lengthViolation.message"));
		// return false;
		// }else{
		// var tr = chkTravelHKPass(appHkid.trim());
		...
		$(
				'input[id^="childselectBenificiary"],input[id^="adultsselectBenificiary"],input[id^="otherSelectBenificiary"] ')
				.each(function(index) {
					$(this).val('SE');
				});
		...
		// var fullname = $(this).val();
		// if (fullname.trim() == "" &&
		// $(this).parent().parent().hasClass('hide') == false) {
		// $("#erradultBenefitiaryId"+errNo).html( getBundle(getBundleLanguage,
		// "beneficiary.name.notNull.message"));//"Please enter your Name in
		// English.";
		// return false;
		// }
		// $("#erradultBenefitiaryId"+errNo).html('');
		// });
		// $( "input[id^='adultBenefitiaryHKId']" ).on( "change blur",
		// function() {
		// var errNo = $(this).attr('id').split("adultBenefitiaryHKId").pop();
		//				
		// var appHkid = $(this).val();
		//				
		// if($('#selectAdBenefitiaryHkidPass'+errNo).length > 0 &&
		// $('#selectAdBenefitiaryHkidPass'+errNo).val().toLowerCase() ==
		// 'passport'){
		//
		// if (appHkid.trim() == "") {
		// $("#erradultBenefitiaryHKId"+errNo).html(getBundle(getBundleLanguage,
		// "beneficiary.passport.notNull.message"));
		// return false;
		// }else{
		// var tr = chkTravelHKPass(appHkid.trim());
		// if (tr == false) {
		// $("#erradultBenefitiaryHKId"+errNo).html(getBundle(getBundleLanguage,
		// "beneficiary.passport.lengthViolation.message"));
		//							
		// return false;
		// }
		// }
		...
		// if (appHkid.trim() == "") {
		// $("#erradultBenefitiaryHKId"+errNo).html(getBundle(getBundleLanguage,
		// "beneficiary.hkId.notNull.message"));//"Please enter your Name in
		// English.";
		// return false;
		// }
		// var tr=IsHKID(appHkid.trim());
		...
		// $( "input[id^='childBenefitiaryName']" ).on( "change blur",
		// function() {
		...
		//				
		//				
		// if($('#selectChldBenefitiaryHkidPass'+errNo).length > 0 &&
		// $('#selectChldBenefitiaryHkidPass'+errNo).val().toLowerCase() ==
		// 'passport'){
		//
		// if (appHkid.trim() == "") {
		// $("#errtxtchildInsuHkid"+errNo).html(getBundle(getBundleLanguage,
		// "beneficiary.passport.notNull.message"));
		// return false;
		// }else{
		// var tr = chkTravelHKPass(appHkid.trim());
		...
		// $( "input[id^='otherBenefitiaryName']" ).on( "change blur",
		// function() {
		...
		// var appHkid = $(this).val();
		//				
		// if($('#selectOtherBenefitiaryHkidPass'+errNo).length > 0 &&
		// $('#selectOtherBenefitiaryHkidPass'+errNo).val().toLowerCase() ==
		// 'passport'){
		//
		// if (appHkid.trim() == "") {
		// $("#errtxtOtherBenInsuHkid"+errNo).html(getBundle(getBundleLanguage,
		// "beneficiary.passport.notNull.message"));
		// return false;
		// }else{
		// var tr = chkTravelHKPass(appHkid.trim());
		...
		$('select[id^="selectOtherAgeRange"],select[id^="selectAgeRange"]')
				.each(function(index) {
					$(this).val('2');
				});
		...
		$('select[id^="selectAgeRange"]').on(
				"change",
				function(index) {
					var errNo = $(this).attr('id').split("selectAgeRange")
							.pop();
		...
		$('select[id^="selectchildAgeRange"]').on(
				"change",
				function(index) {
					var errNo = $(this).attr('id').split("selectchildAgeRange")
							.pop();
					if ($(this).val() == '' || $(this).val() == 0) {
		...
		$('select[id^="selectOtherAgeRange"]').on(
				"change",
				function(index) {
					var errNo = $(this).attr('id').split("selectOtherAgeRange")
							.pop();
					if ($(this).val() == '' || $(this).val() == 0) {
		...
		/*
		 * if($('#selectAgeRange1').length > 0){ $('#selectAgeRange1').val('2');
...
function alphaOnly(evt) {
	...
	if ((evt.charCode == 0 && evt.keyCode == 37)
			|| (evt.charCode == 0 && evt.keyCode == 39)
			|| (evt.charCode == 0 && evt.keyCode == 46)) {
		return true;
	}
	if (charCode > 32 && (charCode < 65 || charCode > 90)
			&& (charCode < 97 || charCode > 122)) {
...
function flightValidateDesk() {
	var flag = true;
	flag = flightValidateGetQuote("txtStartDateDesk", "startDateDeskIn",
			"txtEndDateDesk", "endDateDeskIn", "txtTravellersDesk",
			"lblPeopleDesk", "travelCountDeskIn");
...
function flightValidateMob() {
	...
	flag = flightValidateGetQuote("txtStartDateMob", "startDateMobIn",
			"txtEndDateMob", "endDateMobIn", "txtTravellersMob",
			"lblPeopleMob", "travelCountMobIn");
	return flag;
...
function flightValidateBtm() {
	...
	flag = flightValidateGetQuote("txtStartDateBtm", "startDateBtmIn",
			"txtEndDateBtm", "endDateBtmIn", "txtTravellersBtm",
			"lblPeopleBtm", "travelCountBtmIn");
	return flag;

}
function flightValidateGetQuote(depDateId, errDepDateId, returnDateId,
		errReturnDateId, travellersId, peopleCountId, errTravelCountId) {
	var flag = true;
	...
	flag = chkValidFlightDate(returnDateElement, errReturnDateId, getBundle(
			...
			depDateId, errDepDateId, getBundle(getBundleLanguage,
					"date.policy.startDate.notValid.message"));
	if (travellers > 0) {
		...
		if (travellers.trim() == "" || travellers == "0") {
			...
			var msg = getBundle(getBundleLanguage,
					"flight.traveller.notNull.message");
		...
		if (peopleCount.trim() > 15) {
			...
			var msg = getBundle(getBundleLanguage,
					"flight.traveller.notValid.message");
			document.getElementById(errTravelCountId).innerHTML = msg;
			flag = false;
		}
	}

	if ($('#' + depDateId).closest('form').find(
			'input:radio[name=planSelected]:checked').val() == 'personal') {
...
function flightValidateDeskTravel() {
	...
	var new_start = new Date(startDates[2], startDates[1] - 1, startDates[0],
			0, 0, 0, 0);

	var endDates = new Array();
	endDates = endDate.split("-");
	var new_end = new Date(endDates[2], endDates[1] - 1, endDates[0], 0, 0, 0,
			0);
	...
	if (travelType.trim() == "") {
		$('#travelTypeDeskIn').html(
				getBundle(getBundleLanguage,
						"travel.travelType.notSelected.message"));
		flag = false;
	}

	if (startDate.trim() == "") {
		$('#startDateDeskIn').html(
				getBundle(getBundleLanguage,
						"date.policy.startDate.notValid.message"));
		flag = false;
	} else {
		if (startdays > 90) {
			$('#startDateDeskIn')
					.html(
							getBundle(getBundleLanguage,
									"travelcare.policy.startDate.moreThan30Days.message"));
			flag = false;
		}
	}
	if (travelType != "annual") {
		if (endDate.trim() == "") {
			$('#endDateDeskIn').html(
					getBundle(getBundleLanguage,
							"date.policy.endDate.notValid.message"));
			flag = false;
		} else {
			if (enddays > 180) {
				$('#endDateDeskIn').html(
...
function flightValidateDeskWorkingHoliday() {
	...
	/*
	 * var startDate = document.getElementById("txtStartDateDesk").value; var
	 * endDate = document.getElementById("txtEndDateDesk").value; var
	 * workingholidayers =
	 * document.getElementById("txtworkingholidayersDesk").value;
	 */
...
function flightValidateBtmWorkingHoliday() {
	...
	/*
	 * var startDate = document.getElementById("txtStartDateBtm").value; var
	 * endDate = document.getElementById("txtEndDateBtm").value; var
	 * workingholidayers =
	 * document.getElementById("txtworkingholidayersBtm").value;
	 */
	var startDate = $("#txtStartDateBtm").val();
	var endDate = $("#txtEndDateBtm").val();
...
function validateJoinUsForm() {
	...
	if (fullName.trim() == "") {
		document.getElementById("errorEmptyName").innerHTML = getBundle(
				getBundleLanguage, "memeber.name.notNull.message"); // "Please
																	// enter
																	// your Name
																	// in
																	// English.";
	...
	if (mobileNo.trim() == "") {
		document.getElementById("errorEmptyMob").innerHTML = getBundle(
				getBundleLanguage, "memeber.mobileNo.notNull.message"); // "Please
																		// enter
																		// your
																		// Mobile
																		// No.";
		...
		if (mobile_pattern.test(mobileNo) == false) {
			document.getElementById("errorEmptyMob").innerHTML = getBundle(
					getBundleLanguage, "memeber.mobileNo.notValid.message"); // "Please
																				// enter
																				// an
																				// 8-digit
																				// Mobile
	...
	if (emailId.trim() == "") {
		document.getElementById("errorEmptyEmailId").innerHTML = getBundle(
				getBundleLanguage, "memeber.email.notNull.message"); // "Please
																		// enter
																		// your
																		// Email
																		// Address.";
		...
		if (emailreg.test(emailId) == false) {
			document.getElementById("errorEmptyEmailId").innerHTML = getBundle(
					getBundleLanguage, "memeber.email.notValid.message"); // "Please
																			// enter
																			// a
																			// valid
																			// Email
	...
	// UserName Validation
	if (userName.trim() == "") {
		document.getElementById("errorEmptyUName").innerHTML = getBundle(
				getBundleLanguage, "memeber.username.notNull.message"); // "Please
																		// enter
																		// your
																		// Username.";
		...
		if (reg_user.test(userName) == false) {
			document.getElementById("errorEmptyUName").innerHTML = getBundle(
					getBundleLanguage,
					"memeber.username.notValidLength.message"); // "Username
																// must be
																// between 6 and
																// 50
																// characters.";
	...
	var message = "";
	if (password.trim() == "") {
		message = getBundle(getBundleLanguage,
				"memeber.password.notNull.message"); // "Please enter your
														// Password.";
		$('#txtPass1').addClass('invalid-field');
		valid = false;
	} else if (!rg.test(password)) {
		message += getBundle(getBundleLanguage,
				"memeber.password.notValidLength.message"); // "Password must be
															// at least 8
															// characters and
															// alphanumeric
															// (both upper AND
		...
		message += getBundle(getBundleLanguage,
				"memeber.password.notValid.message"); // "\n Password must be
	...
	if (confirmPassword.trim() == "") {
		document.getElementById("errorEmptyConfPass").innerHTML = getBundle(
				getBundleLanguage, "memeber.confirmPassword.notNull.message"); // "Please
																				// Confirm
																				// Password.";
		...
		if (confirmPassword !== password) {
			document.getElementById("errorEmptyConfPass").innerHTML = getBundle(
					getBundleLanguage,
					"memeber.confirmPassword.notMatch.message"); // "Passwords
																	// you
																	// entered
																	// do not
																	// match.
	...
	if (document.getElementById("checkbox1").checked == false) {
		document.getElementById("chk1").innerHTML = getBundle(
				getBundleLanguage, "memeber.declaration.tnc.notChecked.message"); // "Please
																					// read
																					// and
																					// accept
																					// the
	...
	if (document.getElementById("checkbox2").checked == false) {
		document.getElementById("chk2").innerHTML = getBundle(
				getBundleLanguage,
				"memeber.declaration.pics.notChecked.message"); // "Please read
																// and accept
																// the Personal
																// Information
																// Collection
...
function resetPassword() {
	...
	if (pass.trim() == "") {
		...
		flag = false;
		document.getElementById("errpass").innerHTML = getBundle(
				getBundleLanguage, "memeber.password.notNull.message"); // "Please
																		// enter
																		// a
																		// Password";
	} else {
		if (password_pattern.test(pass) == false) {
			document.getElementById("errpass").innerHTML = getBundle(
					getBundleLanguage, "memeber.password.notValid.message"); // "Please
																				// enter
																				// a
																				// Valid
																				// Password";
	...
	if (confpass.trim() == "") {
		document.getElementById("errconfpass").innerHTML = getBundle(
				getBundleLanguage, "memeber.confirmPassword.notNull.message"); // "Please
																				// enter
																				// a
																				// Confirm
		...
		if (confpass !== pass) {
			document.getElementById("errconfpass").innerHTML = getBundle(
					getBundleLanguage,
					"memeber.confirmPassword.notValid.message"); // "Please
																	// enter a
																	// Valid
																	// Confirm
																	// Password";
...
function validUser(formID) {
	...
	if (password.trim() == "") {
		...
		$("#" + formID + " #errPass").html(
				getBundle(getBundleLanguage, "user.password.notNull.message"));
	...
	if (userName.trim() == "") {
		...
		$("#" + formID + " #errUserName").html(
				getBundle(getBundleLanguage, "user.username.empty.message"));
...
// Chat API Calling
/*
 * function zopim_chat_start(lang) { window.$zopim||(function(d,s){var
 ...
 * 
 * $zopim(function() { $zopim.livechat.setLanguage(lang); }); }
 * zopim_chat_start('en');
 */
...
function validatecardnumber(cardnumber) {
	...
	// var match =
	// /^(?:(4[0-9]{12}(?:[0-9]{3})?)|(5[1-5][0-9]{14})|(6(?:011|5[0-9]{2})[0-9]{12})|(3[47][0-9]{13})|(3(?:0[0-5]|[68][0-9])[0-9]{11})|((?:2131|1800|35[0-9]{3})[0-9]{11}))$/.exec(cardnumber);
	...
	if (match && isCreditCard(cardnumber)) {
		...
		for (var i = 1; i < match.length; i++) {
			if (match[i]) {
				...
				// document.getElementById('errcardno').innerHTML = types[i -
				// 1];
				if (i == 1) {
		...
		document.getElementById('errcardno').innerHTML = getBundle(
				getBundleLanguage, "applicant.creditcard.notValid.message");// '(invalid
																			// card
																			// number)';
...
function payValid() {
	...
	if (cardno.length < 16) {
		...
		$('#errcardno').html(
				getBundle(getBundleLanguage,
						"payment.creditCard.number.notValid.message"));
		$("#cardnumber").addClass("invalid-field");
		if (firstErrorElementId == "") {
	...
	if (!isCreditCard(cardno)) {
		...
		$('#errcardno').html(
				getBundle(getBundleLanguage,
						"applicant.creditcard.notValid.message"));
		$("#cardnumber").addClass("invalid-field");
		if (firstErrorElementId == "") {
			firstErrorElementId = "cardnumber";
		}
	...
	if (month == "" || month == 0) {
		...
		$('#errmonth')
				.html(
						getBundle(getBundleLanguage,
								"payment.creditCard.expiryDate.month.notValid.message"));
	...
	if (year == "" || year == 0) {
		...
		$('#erryear').html(
				getBundle(getBundleLanguage,
						"payment.creditCard.expiryDate.year.notValid.message"));
	...
	if (holdername.trim() == "") {
		...
		$('#errname').html(
				getBundle(getBundleLanguage,
						"payment.creditCard.name.notValid.message"));
	...
	if (seccode.trim() == "") {
		...
		$('#errcode').html(
				getBundle(getBundleLanguage,
						"payment.creditCard.securityCode.notNull.message"));
		$("#seccode").addClass("invalid-field");
		if (firstErrorElementId == "") {
	...
	if (document.getElementById("checkbox3").checked == false) {
		$('#errchk1').html(
				getBundle(getBundleLanguage, "payment.tnc.notChecked.message"));
		flag = false;
		if (firstErrorElementId == "") {
...
function get_promo_val() {
	...
	if (emailId.trim() == "") {
		...
		document.getElementById("errPromoEmail").innerHTML = getBundle(
				getBundleLanguage, "promotion.email.notNull.message");// "Your
																		// E-mail
																		// Address
																		// is
																		// invalid.";
		...
		if (emailreg.test(emailId) == false) {
			document.getElementById("errPromoEmail").style.display = "block";
			document.getElementById("errPromoEmail").innerHTML = getBundle(
					getBundleLanguage, "promotion.email.notValid.message");// "Your
																			// E-mail
																			// Address
																			// is
																			// invalid.";
...
function chkTravelHKPass(value) {
	...
	if (data != "") {
		/*
		 * if (/^[A-Za-z ]+$/.test(data)) { //there are only characters flag =
		 * false; } else if (/^[0-9 ]+$/.test(data)) { //it contains numbers
		 * flag = false; } else
		 */
		if (!filter.test(data)) {
			flag = false;
...
function hc_planValid() {
	...
	/*
	 * document.getElementById("errRegUser").innerHTML = ""; var RegUserName =
	 * document.getElementById("inputRegUserName").value;
	 * document.getElementById("errRegPass").innerHTML = ""; var RegPass =
	 * document.getElementById("inputRegPass").value;
	 * document.getElementById("errRegCPass").innerHTML = ""; var RegCPass =
	 * document.getElementById("inputRegCPass").value;
	 */
	...
	var newEffDate = new Date(EffDates[2], EffDates[1] - 1, EffDates[0], 0, 0,
			0, 0);
	...
	if (appFullName.trim() == "") {
		document.getElementById("appfullname").innerHTML = getBundle(
				getBundleLanguage, "applicant.name.notNull.message");
		$("#inputFullName").addClass("invalid-field");
		if (firstErrorElementId == "") {
			firstErrorElementId = "inputFullName";
	...
	if (mobileNo.trim() == "") {
		document.getElementById("errMobileNo").innerHTML = getBundle(
				getBundleLanguage, "applicant.mobileNo.notNull.message");
		$("#inputMobileNo").addClass("invalid-field");
		if (firstErrorElementId == "") {
			firstErrorElementId = "inputMobileNo";
	...
	if (EmailId.trim() == "") {
		document.getElementById("errEmailid").innerHTML = getBundle(
				getBundleLanguage, "applicant.email.notNull.message");
		$("#inputEmailId").addClass("invalid-field");
		if (firstErrorElementId == "") {
			firstErrorElementId = "inputEmailId";
	...
	/*
	 * if (RegUserName.trim() != "") { if (reg.test(RegUserName) == false) {
	 * document.getElementById("errRegUser").innerHTML = "Please enter an
	 * Username"; flag = false; } } if (RegPass.trim() != "") { if
	 * (password_pattern.test(RegPass) == false) {
	 * document.getElementById("errRegPass").innerHTML = "Please enter a Valid
	 * Password"; flag = false; } if (RegCPass !== RegPass) {
	 * document.getElementById("errRegCPass").innerHTML = "Password does not
	 * match"; flag = false; } }
	 */
	if (CABuilding.trim() == "" && CAEstate.trim() == "") {
		// document.getElementById("errCABuilding").innerHTML = "Name of
		// Building is invalid.";
		$('#errCABuilding')
				.html(
						getBundle(getBundleLanguage,
								"homecare.correspondingAddress.building.notNull.message"));
		$('#errCAEstate')
				.html(
						getBundle(getBundleLanguage,
								"homecare.correspondingAddress.estate.notNull.message"));
	...
	if ($('#selectCADist').val() == '') {
		$('#errCADist').html(
				getBundle(getBundleLanguage,
						"homecare.district.notNull.message"));
	...
	if (ABuilding.trim() == "" && AEstate.trim() == "") {
		// document.getElementById("errABuilding").innerHTML = "Please enter
	...
	if (NFA.trim() == "") {
		// document.getElementById("errNFA").innerHTML = "Please select Net
		// Floor Area.";
		$('#errNFA').html(
				getBundle(getBundleLanguage,
						"homecare.netFloorArea.notNull.message"));
	...
	if (EffDate.trim() == "") {
		// document.getElementById("errEffDate").innerHTML = "Effective Date
		// must be within 60 days of Application Date.";
		$('#errEffDate').html(
				getBundle(getBundleLanguage,
						"homecare.effectiveDate.notValid.message"));
	...
	if (document.getElementById("checkbox1").checked == false) {
		// document.getElementById("chk1").innerHTML = "Please read and accept
		// the Declaration, Terms & Conditions before submitting the
		// application.";
		$('#chk1').html(
				getBundle(getBundleLanguage,
						"travelcare.declaration.notChecked.message"));
		if (firstErrorElementId == "") {
			firstErrorElementId = "checkbox1";
		}
		flag = false;
	...
	if (document.getElementById("checkbox2").checked == false) {
		// document.getElementById("chk2").innerHTML = "Please read and accept
		// the Personal Information Collection Statement before submitting the
		// application.";
		$('#chk2')
				.html(
						getBundle(getBundleLanguage,
								"homecare.tnc.notChecked.message"));
		if (firstErrorElementId == "") {
			firstErrorElementId = "checkbox2";
		}
		flag = false;
		...
		$('#oldHome').on(
				...
				function(e) {
					var newHeight = ($(window).height() / 2)
							- ($('#homecareLandingModal').height() / 2);
					$('#homecareLandingModal').css({
						'top' : newHeight + "px"
					});
				})
...
/*
 * $('#oldHome').on('show.bs.modal', function (event) { var modal = $(this);
 * modal.find('.errorMsg').text(getBundle(getBundleLanguage,
 * "homecare.uw.question1.notEntitled.message")); })
 */
...
function chkValidFlightDepartureDate(element, errElementId, name) {
	...
	if (chkValidDate(element, errElementId, name)) {
		...
		departureDate = new Date(departureDates[2], departureDates[1] - 1,
				departureDates[0], 0, 0, 0, 0);
		// departureDate = new Date(departureDate);
		var dateDiff = dateDiffInDaysFromNow(departureDate);

		if (dateDiff < 0) {
			var msg = getBundle(getBundleLanguage,
					"flight.departureDate.notLessThanCurrent.message");
			document.getElementById(errElementId).innerHTML = msg;
			return false;
		}
		if (dateDiff > 90) {
			var msg = getBundle(getBundleLanguage,
					"flight.departureDate.notValid.message");
			document.getElementById(errElementId).innerHTML = msg;
			return false;
		} else {
			// resetErrElement(errElementId);
...
function chkValidFlightDate(element, errElementId, name, departureDateId,
		errDepartureDateId, departureDateName) {
	...
	// document.getElementById("startDateDeskIn").innerHTML =
	// getBundle(getBundleLanguage, "date.policy.startDate.notValid.message");
	// flag = false;
	// }
	// if (returnDate.trim() == "") {
	// document.getElementById("endDateDeskIn").innerHTML =
	// getBundle(getBundleLanguage, "date.policy.endDate.notValid.message");
	// flag = false;
	// }
	//	
	/*
	 * var departureDate = document.getElementById(departureDateId).value; var
	 * returnDate = element.value;
	 */
	...
	if (chkValidFlightDepartureDate(elementDepartureDate, errDepartureDateId,
			departureDateName)) {
		// check return date
		if (chkValidDate(element, errElementId, name)) {
			...
			returnDate = new Date(returnDates[2], returnDates[1] - 1,
					returnDates[0], 0, 0, 0, 0);
			...
			if (dateDiff < 0) {
				var msg = getBundle(getBundleLanguage,
						"flight.returnDate.notLessThanCurrent.message");
				document.getElementById(errElementId).innerHTML = msg;
				flag = false;
			}
			if (dateDiff > 31) {
				var msg = getBundle(getBundleLanguage,
						"flight.returnDate.notValid.message");
				document.getElementById(errElementId).innerHTML = msg;
				flag = false;
			}
		} else {
...
function chkNotNullBeneficiary(element, errElementId) {
	if (isNull(element)) {
		var msg = getBundle(getBundleLanguage,
				"insured.beneficiary.notNull.message");
		document.getElementById(errElementId).innerHTML = msg;
		$(element).addClass('invalid-field');
		return false;
	} else {
...
function chkValidCreditCard(element, errElementId) {
	if (isNull(element)) {
		var msg = getBundle(getBundleLanguage,
				"applicant.creditcard.notNull.message");
		document.getElementById(errElementId).innerHTML = msg;
		return false;
...
function chkValidCreditCardExpDate(element, errElementId, monthId,
		errMonthElementId) {
	var month = 0;
	var year = 0;
	var now = new Date();
	...
	if (monthId != "") { // idType is id of control
		...
		if (!IsNumeric(month)) {
			var msg = getBundle(getBundleLanguage,
					"applicant.creditcard.month.notNull.message");
			document.getElementById(errMonthElementId).innerHTML = msg;
			$("#inputMonth").addClass("invalid-field");
			return false;
		} else if (month < 1) {
			var msg = getBundle(getBundleLanguage,
					"applicant.creditcard.month.notValid.message");
			document.getElementById(errMonthElementId).innerHTML = msg;
			$("#inputMonth").addClass("invalid-field");
			return false;
		} else {
	...
	if (isNull(element)) {
		var msg = getBundle(getBundleLanguage,
				"applicant.creditcard.year.notNull.message");
		document.getElementById(errElementId).innerHTML = msg;
		$("#inputYear").addClass("invalid-field");
		return false;
	} else {
		...
		if (!IsNumeric(year)) {
			var msg = getBundle(getBundleLanguage,
					"applicant.creditcard.year.notValid.message");
			document.getElementById(errElementId).innerHTML = msg;
			$("#inputYear").addClass("invalid-field");
			return false;
		} else {
...
function chkNotNullCreditCareName(element, errElementId) {
	...
	if (isNull(element)) {
		var msg = getBundle(getBundleLanguage,
				"applicant.creditcard.name.notNull.message");
		document.getElementById(errElementId).innerHTML = msg;
		$("#holdername").addClass("invalid-field");
		return false;
	} else if (element.value.length < 7
			&& element.value.trim().indexOf(" ") > 0) {
		var msg = getBundle(getBundleLanguage,
				"applicant.creditcard.noSpaces.message");
		document.getElementById(errElementId).innerHTML = msg;
		$("#holdername").addClass("invalid-field");
		return false;
	} else {
...
function chkNotNullCABuilding(element, errElementId) {
	document.getElementById(errElementId).innerHTML = '';
	// COMMENT BY NAT - AS THE RULE NOW IS EITHER BUILDING / ESTATE NEED TO
	// FILLED
	...
	// if(isNull(element)){
	// var msg = getBundle(getBundleLanguage,
	// "applicant.address.correspondence.building.notNull.message");
	// document.getElementById(errElementId).innerHTML = msg;
	// return false;
	// }else{
	// resetErrElement(errElementId);
...
function chkNotNullCAEstate(element, errElementId) {
	...
	// if(isNull(element)){
	// var msg = getBundle(getBundleLanguage,
	// "applicant.address.correspondence.estate.notNull.message");
	// document.getElementById(errElementId).innerHTML = msg;
	// return false;
	// }else{
	// resetErrElement(errElementId);
...
function chkNotNullIABuilding(element, errElementId) {
	...
	// if(isNull(element)){
	// var msg = getBundle(getBundleLanguage,
	// "insured.address.building.notNull.message");
	// document.getElementById(errElementId).innerHTML = msg;
	// return false;
	// }else{
	// resetErrElement(errElementId);
...
function chkNotNullIAEstate(element, errElementId) {
	...
	// if(isNull(element)){
	// var msg = getBundle(getBundleLanguage,
	// "insured.address.estate.notNull.message");
	// document.getElementById(errElementId).innerHTML = msg;
	// return false;
	// }else{
	// resetErrElement(errElementId);
...
function chkNotNullIANetFloorArea(element, errElementId) {
	if (isNull(element)) {
		var msg = getBundle(getBundleLanguage,
				"insured.address.netFloorArea.notNull.message");
		document.getElementById(errElementId).innerHTML = msg;
		$("#inputNFA").addClass("invalid-field");
		return false;
	} else {
...
function chkValidIAEffDate(element, errElementId, name) {
	if (chkValidDate(element, errElementId, name)) {
		...
		if (startdays > 60) {
			var msg = getBundle(getBundleLanguage,
					"insured.address.effectiveDate.notValid.message");
			document.getElementById(errElementId).innerHTML = msg;
			return false;
		} else {
			resetErrElement(errElementId);
...
function passMatch(pass, confirm) {
	if (confirm.trim() == "") {
		return getBundle(getBundleLanguage,
				"user.confirmPassword.empty.message");
	} else if (confirm !== pass) {
		return getBundle(getBundleLanguage,
				"user.confirmPassword.validate.message");
	} else {
		return true;
	}
}
...
function checkMembership(field) {
	...
	if (field == "Username") {
		...
		if (value == '') {
			...
			$("#Password").addClass("invalid-field");
			$('#PasswordError').text(
					getBundle(getBundleLanguage, "user.password.same.message"));
			result = false;
		} else {
			$('#PasswordError').text('');
			$("#Password").removeClass("invalid-field");
...
<<<<<<< .mine
<<<<<<< HEAD
 * 楠岃瘉鍖呭惈鐨?@"鍜?.",鍚勮嚜鍙兘鏈変竴涓笖涓嶈兘涓哄紑澶村拰缁撳熬
=======
...
function checkUsername(el) {
	if (el.split('@').length - 1 > 1) {
		...
		/*
		 * }else if (el.split('.').length-1 > 1){ return
		 * getBundle(getBundleLanguage, 'user.username.only46.message');
		 */
	} else if (el.indexOf('@') == 0 || el.lastIndexOf('@') == el.length - 1) {
		return getBundle(getBundleLanguage,
				'user.username.beginorover64.message');
	} else if (el.indexOf('.') == 0 || el.lastIndexOf('.') == el.length - 1) {
		return getBundle(getBundleLanguage,
				'user.username.beginorover46.message');
	}
	return true
}
	...
	$('#txtFullName').on(
			...
			function() {
				if ($(this).val() == '') {
					$('#errorEmptyName').text(
							getBundle(getBundleLanguage,
									"membership.fullName.empty.message"));
					$('#txtFullName').addClass('invalid-field');
				} else {
					$('#errorEmptyName').text('');
				}
	...
	$cur.on('blur',
			function() {
				...
				if (isValidPassword(value) !== true) {
					...
					$('#errorJoinUsPassword').text(
							getBundle(getBundleLanguage,
									"user.password.same.message"));
					$('#txtPass1').addClass('invalid-field');
				} else
					$('#errorJoinUsPassword').text('');
			})
...
function activateUserAccount() {
	...
	if (name == '') {
		...
		if (name == password) {
			$('#errorEmptyName').text(
					getBundle(getBundleLanguage,
							"membership.fullName.equal.password.message"));
			$("#txtFullName").addClass("invalid-field");
			if (firstErrorElementId == "") {
				firstErrorElementId = "txtFullName";
			}
	...
	if (!declaration) {
		$('#errorDeclaration').text(
				getBundle(getBundleLanguage,
						"membership.declaration.notChecked.message"));
		if (firstErrorElementId == "") {
			firstErrorElementId = "checkbox1";
		}
		check = false;
...
function hkidOnkeypress(evt) {
	...
	if ((charCode >= 48 && charCode <= 57)
			...
			|| (charCode >= 97 && charCode <= 122) || charCode == 127
			|| charCode == 8 || (evt.charCode == 0 && evt.keyCode == 37)
			|| (evt.charCode == 0 && evt.keyCode == 46)
			|| (evt.charCode == 0 && evt.keyCode == 39)) {
		return true;
	}
...
$(function() {
	...
	var now = new Date(nowTemp.getFullYear(), nowTemp.getMonth(), nowTemp
			.getDate(), 0, 0, 0, 0);
	var tillDate_from_home = new Date((new Date()).getTime() + 59 * 24 * 60
			* 60 * 1000);
	...
	var checkin = $('#homecareDp').datepicker(
			{
				beforeShowDay : function(date) {
					return date.valueOf() >= now.valueOf()
							&& date.valueOf() < tillDate_from_home;
				},
				autoclose : true,
				todayHighlight : true,
				format : "dd-mm-yyyy"
...
function validationUsername(evt) {
	...
	if ((charCode >= 48 && charCode <= 57)
			...
			|| charCode == 8 || (evt.charCode == 0 && evt.keyCode == 37)
			|| (evt.charCode == 0 && evt.keyCode == 46)
			|| (evt.charCode == 0 && evt.keyCode == 39) || charCode == 45
			|| charCode == 95 || charCode == 46 || charCode == 64) {
		return true;
	}
	return false;
}
...
try {
	$("#txtUserName1").unbind("keyup");
} catch (err) {
}
try {
	$("#fUserName").unbind("keyup");
} catch (err) {
}
try {
	$("#headerUserName").unbind("keyup");
} catch (err) {
}
try {
	$("#userName").unbind("keyup");
...
window.jQuery(function() {
	...
	var scrollDiv = $('<div class="scrollbar-measure"></div>').appendTo(
			document.body)[0], scrollBarWidth = scrollDiv.offsetWidth
...
function mergeCreditCard() {
	var creditcardNumber = $("#cardNo1").val() + $("#cardNo2").val()
			+ $("#cardNo3").val() + $("#cardNo4").val();
	$("#cardnumber").val(creditcardNumber);

	setTimeout(function() {
		if (!$("#cardNo1").is(":focus") && !$("#cardNo2").is(":focus")
				&& !$("#cardNo3").is(":focus") && !$("#cardNo4").is(":focus")) {
