...
public class TravelController {
	...
	public ModelAndView getTravelHomePage(HttpServletRequest request,
			Model model) {
...
//			
			...
			String Url = UserRestURIConstants.TRAVEL_GET_QUOTE + "?planCode=A"
					...
					+ expiryDate + "&planCode=" + userReferralCode;
	...
	public String applyPromotionCode(
		...
		try {
			...
			if (responseJsonObj.get("errMsgs") == null) {
				...
				String planeName[] = { "Plan A", "Plan B" };
				String grossPrem[] = {
						StringHelper.formatPrice(jsonPriceInfoA.get(
								"grossPremium").toString()),
						StringHelper.formatPrice(jsonPriceInfoB.get(
								"grossPremium").toString()) };
				...
				String totalNetPremium[] = {
						StringHelper.formatPrice(jsonPriceInfoA.get(
								"totalNetPremium").toString()),
						StringHelper.formatPrice(jsonPriceInfoB.get(
								"totalNetPremium").toString()) };

				String totalDue[] = {
						StringHelper.formatPrice(jsonPriceInfoA.get("totalDue")
								.toString()),
						StringHelper.formatPrice(jsonPriceInfoB.get("totalDue")
								.toString()) };
			...
			model.addAttribute("quoteDetails",
					session.getAttribute("quoteDetails"));
			model.addAttribute("travelQuoteBean", travelQuote);
	...
	public ModelAndView prepareYourDetails(
		...
		String planSummary = WebServiceUtils.getParameterValue(
				"selectedAmountDue", session, request);
		String selectPlanName = WebServiceUtils.getParameterValue("planName",
				session, request);
		...
		try {
			...
			QuoteDetails quoteDetails = (QuoteDetails) session
					.getAttribute("quoteDetails");
			if ("Plan A".equals(selectPlanName)) {
				session.setAttribute("planSelected", "A");
				model.addAttribute("planDiscount",
						quoteDetails.getDiscountAmount()[0]);
				model.addAttribute("planSummary",
						quoteDetails.getTotalNetPremium()[0]);
			} else {
				session.setAttribute("planSelected", "B");
				model.addAttribute("planDiscount",
						quoteDetails.getDiscountAmount()[1]);
				model.addAttribute("planSummary",
						quoteDetails.getTotalNetPremium()[1]);
			}
	...
	public ModelAndView prepareSummary(
		...
		String dueAmount = "";
		if (selectPlanName.equals("Plan A")) {
			JSONObject jsonPriceInfoA = (JSONObject) session
					.getAttribute("priceInfoA");
			dueAmount = String.valueOf(jsonPriceInfoA.get("totalDue"));

		}

		else {
			JSONObject jsonPriceInfoB = (JSONObject) session
					.getAttribute("priceInfoB");
			dueAmount = String.valueOf(jsonPriceInfoB.get("totalDue"));
